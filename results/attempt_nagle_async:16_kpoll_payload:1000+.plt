set term pngcairo size 1600, 1000 font 'Tahoma,10' enhanced; set output 'attempt_nagle_async:16_kpoll_payload:1000+.png'
set title 'attempt nagle async:16 kpoll payload:1000+'
set termoption dash
set dummy jw,y
...
plot 'attempt_nagle_async:16_kpoll_payload:1000+.log' using 1:2 lt 1 lc rgb 'dark-green' lw 2 with lines title 'rate' axes x1y1,\
     'attempt_nagle_async:16_kpoll_payload:1000+.log' using 1:3 lt 1 lc rgb 'dark-red' lw 2 with lines title 'delay ms' axes x2y2,\
     'attempt_nagle_async:16_kpoll_payload:1000+.log' using 1:4 lt 1 lc rgb 'dark-blue' with lines title 'tx packet rate',\
     'attempt_nagle_async:16_kpoll_payload:1000+.log' using 1:5 lt 5 lc rgb 'dark-blue' with lines title 'rx packet rate',\
     'attempt_nagle_async:16_kpoll_payload:1000+.log' using 1:($6/1024) lt 1 lc rgb 'dark-gray' with lines title 'tx kbyte rate',\
     'attempt_nagle_async:16_kpoll_payload:1000+.log' using 1:($7/1024) lt 5 lc rgb 'dark-gray' with lines title 'rx kbyte rate'

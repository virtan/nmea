set term pngcairo size 1600, 1000 font 'Tahoma,10' enhanced; set output 'attempt_nagle_async:4_kpoll_payload:10+.png'
set title 'attempt nagle async:4 kpoll payload:10+'
set termoption dash
set dummy jw,y
...
plot 'attempt_nagle_async:4_kpoll_payload:10+.log' using 1:2 lt 1 lc rgb 'dark-green' lw 2 with lines title 'rate' axes x1y1,\
     'attempt_nagle_async:4_kpoll_payload:10+.log' using 1:3 lt 1 lc rgb 'dark-red' lw 2 with lines title 'delay ms' axes x2y2,\
     'attempt_nagle_async:4_kpoll_payload:10+.log' using 1:($4*10) lt 1 lc rgb 'dark-blue' with lines title 'tx packet rate *10',\
     'attempt_nagle_async:4_kpoll_payload:10+.log' using 1:($5*10) lt 5 lc rgb 'dark-blue' with lines title 'rx packet rate *10',\
     'attempt_nagle_async:4_kpoll_payload:10+.log' using 1:(10*$6/1024) lt 1 lc rgb 'dark-gray' with lines title 'tx kbyte rate *10',\
     'attempt_nagle_async:4_kpoll_payload:10+.log' using 1:(10*$7/1024) lt 5 lc rgb 'dark-gray' with lines title 'rx kbyte rate *10'
